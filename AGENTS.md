...
- Static assets are `assets/logo.svg`, `favicon.ico`, `favicon.png`, `og-image.svg`, and `og-image.png`. `robots.txt`, `sitemap.xml`, and `vercel.json` are deployment/SEO/support files.
...
- The Content Security Policy currently allows self-hosted scripts/styles/images, Google Fonts, and Vercel tooling.
